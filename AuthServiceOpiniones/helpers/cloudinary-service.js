...
export const uploadImage = async (filePath, fileName) => {
  try {
    ...
    // Log result for debugging (shows public_id, secure_url, etc.)
    console.info('Cloudinary upload result:', {
    ...
    // Return the cloudinary public_id (includes folder when set) so
    // the rest of the app can build the full URL or manage deletion.
    return result.public_id || fileName;
  } catch (error) {
