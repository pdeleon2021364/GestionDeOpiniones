...
export const registerUserHelper = async (userData) => {
  try {
    ...
    if (profilePicture) {
        ...
        // Detectar si es un archivo local (soporta backslashes y paths absolutos)
        const normalizedUploadPath = path
          .normalize(uploadPath)
          .replace(/^\.[/\\]?/, '');
        const normalizedProfile = path.normalize(profilePicture);
        const isLocalFile =
          path.isAbsolute(profilePicture) ||
          normalizedProfile.includes(normalizedUploadPath) ||
          normalizedProfile.startsWith('.') ||
          normalizedProfile.startsWith(uploadPath.replace(/^[.\\\/]+/, '')) ||
          normalizedProfile.includes('uploads');
      ...
      if (isLocalFile) {
        try {
          ...
          console.info('Uploading local file to Cloudinary:', {
            localPath: profilePicture,
            cloudinaryFileName,
